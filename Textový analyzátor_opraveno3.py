...
def display_registered_users(users):
    print("+------+-------------+")
    print("| user |   password  |")
    print("+------+-------------+")
...
def analyze_text(text):
    ...
    uppercase_words = sum(1 for word in words if word.isalpha() and word.isupper())
    ...
    print(f"There are {titlecase_words} title case words.")
...
def main():
    ...
    if username in USERS and USERS[username] == password:
        ...
        if select.isdigit() and 1 <= int(select) <= len(TEXTS):
            analyze_text(TEXTS[int(select) - 1])
        else:
